...
def dp_backend_info(*, time_model: SegmentTimeModel, device: str | None = "auto") -> dict:
    """Return a small diagnostic dict about whether DP will use GPU.

    In this repository, GPU acceleration is only used for the **trapezoid**
    time model DP, and only when:

    - torch is installed
    - torch reports CUDA available
    - the requested `device` resolves to a CUDA torch.device

    This helper is intentionally lightweight and safe to call in CLI scripts
    (it will not crash if torch/cuda is unavailable).
    """

    requested = "auto" if device is None else str(device).strip()
    effective_time_model = str(getattr(time_model.info, "effective", ""))

    out = {
        "time_model_effective": effective_time_model,
    ...
    # Only trapezoid DP has a torch (CUDA) backend.
    if effective_time_model != "trapezoid":
        out["note"] = f"time_model={effective_time_model} (DP runs on CPU)"
        return out

    torch = _try_import_torch()
    if torch is None:
        out["note"] = "torch unavailable (DP runs on CPU)"
        return out
    ...
    # Resolve requested device (auto/cpu/cuda/cuda:0...)
    torch_device, use_cuda, note = _resolve_torch_device(torch, requested)
    out["note"] = str(note)

    if not use_cuda:
        out["device_effective"] = "cpu"
        out["gpu_used"] = False
        # Count GPUs if possible (useful even if not used)
    ...
    out["gpu_used"] = True
    out["device_effective"] = str(torch_device)

    # Best-effort GPU metadata.
    try:
        out["gpu_count"] = int(torch.cuda.device_count())
    except Exception:
        out["gpu_count"] = 0
    ...
    try:
        idx = 0 if getattr(torch_device, "index", None) is None else int(torch_device.index)
        out["gpu_name"] = str(torch.cuda.get_device_name(idx))
    except Exception:
        out["gpu_name"] = None
...
def _optimal_path_indices_dp_torch(
    *,
    ...
    # NOTE: We copy parent indices GPU->CPU **once per layer** (not per block)
    # to reduce device synchronizations and Python overhead.
    ...
    for li in range(1, len(Q_layers)):
        ...
        parent_gpu = torch.empty((Kcurr,), dtype=torch.int64, device=torch_device)

        # Reuse the broadcasted dp column for this layer.
        dp_col = dp.reshape((-1, 1))
        ...
        for j0 in range(0, Kcurr, block_size):
            ...
            # totals = tmat + dp_col  (in-place to reduce allocations)
            tmat.add_(dp_col)

            # values/indices: (B,)
            dp_block, parent_block = torch.min(tmat, dim=0)

            dp_new[j0:j1] = dp_block
            parent_gpu[j0:j1] = parent_block.to(torch.int64)

        # Copy parent indices back to CPU once per layer.
        parents_cpu.append(parent_gpu.detach().to("cpu").numpy())
        dp = dp_new
...
def _optimal_path_indices_dp(
    *,
    ...
    # DP dtype (CUDA only)
    dp_dtype_raw = "float64" if dp_dtype is None else str(dp_dtype).strip().lower()
    if dp_dtype_raw in {"", "auto"}:
        # Faster default for CUDA, but keep explicit default as float64 for backwards compatibility.
        dtype_torch = torch.float32
    elif dp_dtype_raw in {"float32", "fp32", "32", "single"}:
        dtype_torch = torch.float32
    elif dp_dtype_raw in {"float64", "fp64", "64", "double"}:
    ...
    if _precomputed_Q_torch is not None:
        # Ensure device + dtype match the DP configuration.
        Q_layers = [q.to(device=torch_device, dtype=dtype_torch) for q in list(_precomputed_Q_torch)]
    else:
        layers = [list(layer) for layer in layers]
        Q_np = [np.asarray([s.joint_positions for s in layer], dtype=float) for layer in layers]
        Q_layers = [torch.as_tensor(q, dtype=dtype_torch, device=torch_device) for q in Q_np]
...
def window_path_receding_horizon(
    *,
    ...
    device: str = "auto",
    dp_dtype: str = "float64",
    _precomputed_Q_all_torch: Sequence[Any] | None = None,
) -> PathResult:
    ...
    # We also allow callers (e.g. benchmark scripts) to pass a shared precomputed list.
    Q_all_torch: List[Any] | None = None

    if _precomputed_Q_all_torch is not None:
        Q_all_torch = list(_precomputed_Q_all_torch)
    elif str(time_model.info.effective) == "trapezoid":
        torch = _try_import_torch()
        ...
        if remain <= w:
            ...
            if Q_all_torch is not None:
                ...
                idxs, _best_window_time = _optimal_path_indices_dp(
                    ...
                    block_size=block_size,
                    device=str(device),
                    dp_dtype=str(dp_dtype),
                    _precomputed_Q_torch=Q_window,
                )
            else:
                idxs, _best_window_time = _optimal_path_indices_dp(
                    start_q=current_q,
