...
3) For that window_size, run seeds in [seed-start .. seed-end] (inclusive)
4) For each run, execute:
      ros2 launch panda_ik_window ik_benchmark.launch.py \
        num_points:=8 seed:=7 window_size:=3 \
        data_root:=... \
        time_model:=trapezoid device:=cuda dp_block_size:=256 \
        (plus optional --extra launch args)
5) Parse each run's data_root/<timestamp>/summary.json
   - Each run also evaluates greedy baseline (ws=1) internally and records it in summary.json.
...
def _parse_window_size_arg(ws: int, *, num_points: int) -> int:
    """Parse a single --window-size integer.

    Requirements:
      - ws must be > 1
      - ws is clamped to <= num_points

    Note: greedy baseline (ws=1) is evaluated automatically by the benchmark node.
    """
    n = max(1, int(num_points))
    w = int(ws)
    if w <= 1:
        raise ValueError("--window-size must be an integer > 1 (greedy baseline ws=1 is automatic)")
    if w > n:
        w = n
    if w <= 1:
        raise ValueError(
            f"--window-size must be > 1, but got ws={int(ws)} and num_points={n} (after clamp -> {w})"
        )
    return int(w)
...
def _run_ros2_launch(
    *,
    ...
    cmd.extend(list(extra_launch_args))
...
def main() -> int:
    ...
    ap.add_argument(
        ...
        default=3,
        help=(
            "Window size ws (>1). Greedy baseline (ws=1) is evaluated automatically in each run and "
            "recorded in summary.json. Default: 3."
        ),
    )
    ...
    ap.add_argument(
        ...
        default=1,
        help=(
            "How many runs to execute concurrently (threaded; each run is a separate ros2 launch subprocess). "
            "WARNING: running multiple MoveIt/ROS2 launches concurrently may require setting unique ROS_DOMAIN_IDs. "
    ...
    ap.add_argument(
        ...
        type=str,
        default="trapezoid",
        help="Launch arg time_model (default: trapezoid).",
    )
    ap.add_argument(
        "--device",
        type=str,
    ...
    ws = _parse_window_size_arg(int(args.window_size), num_points=num_points)
    ...
    print(f"  window_size     = {ws} (from --window-size={int(args.window_size)})")
    print(f"  seeds           = {seeds}")
    ...
    # Run seeds (optionally in parallel).
    jobs = max(1, int(args.jobs))
    if jobs == 1:
        seeds_iter = list(seeds)
    else:
        print(f"[batch] running seeds with jobs={jobs} (concurrent subprocesses)")
        seeds_iter = list(seeds)
        ...
        def _run_one_seed(seed: int) -> tuple[int, List[float]]:
            nonlocal round_idx
            with round_lock:
                round_idx += 1
                local_round = int(round_idx)
            print(
                f"\n[batch] 当前轮次/总轮次: {local_round}/{total_rounds} | "
            ...
            if rc != 0:
                ...
                return int(seed), [math.nan] * num_points
    ...
    if jobs == 1:
        for seed in seeds_iter:
            s, rates = _run_one_seed(int(seed))
            rates_by_seed[int(s)] = list(rates)
    else:
        from concurrent.futures import ThreadPoolExecutor, as_completed

        with ThreadPoolExecutor(max_workers=jobs) as ex:
            futs = [ex.submit(_run_one_seed, int(seed)) for seed in seeds_iter]
